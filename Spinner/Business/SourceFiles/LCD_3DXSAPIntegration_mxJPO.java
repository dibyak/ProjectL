...
public class LCD_3DXSAPIntegration_mxJPO extends LCD_Constants_mxJPO {
	...
	public void sendToSAP(Context context, String args[]) throws Exception {
		try {
			...
			ChangeAction changeAction = ChangeActionServices.getChangeAction(context, strCAId);
									...
									for (int i = 0; i < realizedItems.size(); i++) {
										...
										if (null != realizedItem) {
											...
											if (UIUtil.isNotNullAndNotEmpty(realizedItemId)) {
												...
												if (TYPE_MANUFACTURINGASSEMBLY.equalsIgnoreCase(realizedItemType) && DESCRETE.equalsIgnoreCase(strObjProcIntent) && FALSE.equalsIgnoreCase(strHasConfig)) {
	...
	private JsonObjectBuilder getSubContractPartHeader( Context context , Map mSubContractPartDetails ,String strRelId ) throws Exception
	{
		...
		try {
			...
			//String strRelId = (String) mSubContractPartDetails.get(DomainConstants.SELECT_RELATIONSHIP_ID);
	...
	private Map<?, ?> getEffectivity(Context context, String strRelId) throws Exception {
		...
		try {
			...
			mEffectivity = iConfigurationExposedFilterables.getEffectivitiesContent(context, objects, com.dassault_systemes.plm.config.exposed.constants.Domain.ALL,
					com.dassault_systemes.plm.config.exposed.constants.Format.TXT, com.dassault_systemes.plm.config.exposed.constants.View.ALL);
		} catch (Exception ex) {
			ex.printStackTrace();
			throw ex;
		} finally {
	...
	private static int setURL(Context context) throws Exception {
		URL = EnoviaResourceBundle.getProperty(context, LCD_3DXSAPIntegrationKey, "LCD_3DXSAPStringResource_en.SAP.WebServiceURL.DEV", getLanguage());
		return 0;
	}
