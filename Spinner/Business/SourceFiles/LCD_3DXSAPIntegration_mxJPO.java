...
public class LCDSyncToSAP extends LCDConstants {

	private static final Base64.Encoder BASE64_ENCODER = Base64.getEncoder();
	private static final String LCD_3DX_SAP_INTEGRATION_KEY = "LCD_3DXSAPStringResource_en";
		...
		while (iterMAsMaplist.hasNext()) {
			...
			if (!(strProcessStatusFlag.equalsIgnoreCase("Complete") && strProcessStatusFlag.equalsIgnoreCase("In Work"))) {
				if (TYPE_MANUFACTURINGASSEMBLY.equalsIgnoreCase(strBOMComponentType)) {
					...
					String strProcurementIntent = (String) mLinkedCadPart.get(ATTR_PROCUREMENTINTENT_VPMREFERENCE);
	...
	public void SendToSAP(Context context, String strBOMComponentId, String strBOMComponentType, String caId,
			String processStatusFlag) throws Exception {
			...
			if (null != changeAction) {
				...
				if (UIUtil.isNotNullAndNotEmpty(changeActionJsonDetails)) {
					...
					if (null != joChangeAction) {
						...
						if (UIUtil.isNotNullAndNotEmpty(getCSRFToken())
								&& UIUtil.isNotNullAndNotEmpty(getSessionCookie())) {
...
//							JsonObjectBuilder jEachPayloadBuilder = addChangeActionHeader(context);
							...
							if (changeActionMap.containsKey(caId)) {
								...
								jEachPayloadBuilder = getChangeActionJSON(context, caId);
							...
							if (null != jEachPayloadBuilder) {
								...
								if (UIUtil.isNotNullAndNotEmpty(strBOMComponentId)) {
									...
									strObjProcIntent = (String) mobjectDetails
											.get(ATTR_PROCUREMENTINTENT_MANUFACTURINGASSEMBLY);
									strHasConfig = (String) mobjectDetails.get(ATTR_HASCONFIGCONTEXT_VPMREFERENCE);
...
//									strBOMComponentType = (String) mobjectDetails.get(DomainConstants.SELECT_TYPE);
									...
									if (TYPE_MANUFACTURINGASSEMBLY.equalsIgnoreCase(strBOMComponentType) && DESCRETE.equalsIgnoreCase(strObjProcIntent) && FALSE.equalsIgnoreCase(strHasConfig)) {
										strSAPMBOMUpdatedOn = (String) mobjectDetails.get(ATTR_SAPMBOMUPDATEDON_MANUFACTURINGASSEMBLY);
										strSAPUniqueID = (String) mobjectDetails.get(ATTR_SAPUNIQUEID_MANUFACTURINGASSEMBLY);
										...
										if (UIUtil.isNullOrEmpty(strSAPMBOMUpdatedOn) && UIUtil.isNullOrEmpty(strSAPUniqueID)) {
											...
											if (null != jHeaderPartObjectBuilder) {
												...
												jsonLogger.writeLog("JSON Payload Request for Discrete : <<< "+ strObjTitle + " : >>> \n " + requestString + " \n ");
												...
												if (UIUtil.isNotNullAndNotEmpty(jEachPayloadObj.toString())) {
													...
													jsonLogger.writeLog("Response for Discrete : <<< " + strObjTitle + " :  Payload from SAP WebService >>> \n " + responseString
															+ " \n ");
													...
													if (UIUtil.isNotNullAndNotEmpty(responseString))
														...
														logger.writeLog(
																"INFO :: No response received from SAP Webservice for Realized Item << "
																		+ strObjTitle
																		+ " >> is with Procurement Intent : << "
																		+ strObjProcIntent
																		+ " >> from Change Action : << "
																		+ changeActionName + " >> ");
												} else
	 ...
	 * @throws Exception
	 */
	private JsonObjectBuilder getChangeActionJSON(Context context, String caId) throws Exception {
		...
		try {
			...
			// STEP : Retrieving Change Action Details from Input Change Action JSON
			String strCACompletionDate = (String) changeActionAttrDetails.get(RELEASEDDATE_CHANGEACTION);
			...
			if (UIUtil.isNotNullAndNotEmpty((String) changeActionAttrDetails.get(PLATFORM_CHANGEACTION)))
				jEachPayloadBuilder.add(TAG_PLATFORM, (String) changeActionAttrDetails.get(PLATFORM_CHANGEACTION));
...
//				}
			...
			if (UIUtil.isNotNullAndNotEmpty(strObjectID)) {
				...
				if (!mlSubContractList.isEmpty()) {
					...
					for (int kdx = mlSubContractList.size() - 1; kdx >= 0; --kdx) {
						...
						strProcIntent = (String) mSubContractPartDetails.get(ATTR_PROCUREMENTINTENT_VPMREFERENCE);
	...
	private MapList getAllLevelSubContract(Context context, String strObjectId) throws Exception {
		...
		try {
			...
			if (UIUtil.isNotNullAndNotEmpty(strObjectId)) {
				...
				ebomList.sort(ATTR_TREEORDER_PLMINSTANCE, "ascending", "real");
				// STEP : Collecting only BuySubC part from expanded childs
				Map<?, ?> tempMap;
				if (!ebomList.isEmpty()) {
					for (int b = 0; b < ebomList.size(); b++) {
						tempMap = (Map<?, ?>) ebomList.get(b);
						strProcurementIntent = (String) tempMap.get(ATTR_PROCUREMENTINTENT_VPMREFERENCE);
						if (SUBCONTRACT.equalsIgnoreCase(strProcurementIntent)) {
				...
				if (null != jSubContractBuilder) {
					...
					if (mlObjects != null) {
						...
						for (int kdx = 0; kdx < mlObjects.size(); kdx++) {
							...
							strProcIntent = (String) mchildPartDetails.get(ATTR_PROCUREMENTINTENT_VPMREFERENCE);
								...
								if (mlBuySubCObjects != null) {
									for (int idx = 0; idx < mlBuySubCObjects.size(); idx++) {
										...
										strProcIntent = (String) mchildPartDetails
												.get(ATTR_PROCUREMENTINTENT_VPMREFERENCE);
										if (BUYSUBC.equalsIgnoreCase(strProcIntent)) {
											jChildBuilder = addCADChildPartHeader(context, mchildPartDetails);
											if (null != jChildBuilder)
												jsubContractChildPartArrayBuilder.add(jChildBuilder.build());
	...
	private JsonObjectBuilder getSubContractPartHeader(Context context, Map<?, ?> mSubContractPartDetails, String strRelId)
			throws Exception {
		...
		try {
			...
			String strProcurementIntent = (String) mSubContractPartDetails.get(ATTR_PROCUREMENTINTENT_VPMREFERENCE);
			String strPartInterchangeability = (String) mSubContractPartDetails.get(ATTR_PARTINTERCHANGEABILITY_VPMREFERENCE);
			String strServiceable = (String) mSubContractPartDetails.get(ATTR_SERVICEABLEITEM_VPMREFERENCE);
	...
	private MapList getFirstLevelChildren(Context context, String strObjectId) throws Exception {
		...
		try {
			if (UIUtil.isNotNullAndNotEmpty(strObjectId)) {
				...
				mlObjects.sort(ATTR_TREEORDER_PLMINSTANCE, "ascending", "real");
	...
	private JsonObjectBuilder addCADChildPartHeader(Context context, Map<?, ?> mchildPartDetails) throws Exception {
		...
		try {
			...
			String strUnitOfMeasure = (String) mchildPartDetails.get(ATTR_UNITOFMEASURE_VPMREFERENCE);
			String strProcIntent = (String) mchildPartDetails.get(ATTR_PROCUREMENTINTENT_VPMREFERENCE);
			String strServiceItem = (String) mchildPartDetails.get(ATTR_SERVICEABLEITEM_VPMREFERENCE);
			String strPartability = (String) mchildPartDetails.get(ATTR_PARTINTERCHANGEABILITY_VPMREFERENCE);
						...
						if (UIUtil.isNotNullAndNotEmpty(resultType) && resultType.equalsIgnoreCase(FAIL)) {
							...
							if (UIUtil.isNotNullAndNotEmpty(strRelID)) {
								...
								try {
									...
									domRel.setAttributeValue(context, ATTR_SAP_CAD_INSATNCE_UPDATED_ON,
									...
									if (UIUtil.isNotNullAndNotEmpty(sRelID)) {
										...
										try {
											...
											domRelationship.setAttributeValue(context,
													ATTR_SAP_CAD_INSATNCE_UPDATED_ON, DateFormat.format(date));
											ContextUtil.popContext(context);
										} catch (Exception me) {
											throw me;
										}
								...
								try {
									if (getSubContractFailedFlag() == false) {
										...
										ContextUtil.pushContext(context,
												PropertyUtil.getSchemaProperty(context, "person_UserAgent"),
												DomainConstants.EMPTY_STRING, DomainConstants.EMPTY_STRING);
										HeaderObject.setAttributeValue(context, ATTR_SAP_UNIQUEID, strSAPUniqueID);
										HeaderObject.setAttributeValue(context, ATTR_SAPMBOM_UPDATED_ON,
												dateFormat.format(date));
										ContextUtil.popContext(context);
									}
								} catch (Exception e) {
										...
										if (UIUtil.isNotNullAndNotEmpty(sRelID)) {
											...
											try {
												...
												ContextUtil.pushContext(context,
														PropertyUtil.getSchemaProperty(context, "person_UserAgent"),
														DomainConstants.EMPTY_STRING, DomainConstants.EMPTY_STRING);
												domRel.setAttributeValue(context, ATTR_SAP_INSATNCE_UPDATED_ON,
														dateFormat.format(date));
												ContextUtil.popContext(context);
											} catch (Exception me) {
												throw me;
		...
		try {
			...
			if (!realizedItems.isEmpty()) {
				...
				for (int i = 0; i < realizedItems.size(); i++) {
					...
					realizedItemId = realizedItem.getJsonObject("where").get("id").toString().split(":")[1].replace("\"", "");
					if (UIUtil.isNotNullAndNotEmpty(realizedItemId)) {
						...
						if (realizedItemType.equalsIgnoreCase(TYPE_MANUFACTURINGASSEMBLY)
							...
							if (UIUtil.isNotNullAndNotEmpty(strObjId) && UIUtil.isNotNullAndNotEmpty(strResponseStatusLine) && UIUtil.isNotNullAndNotEmpty(strTitle))
	 ...
	 * @throws Exception
	 */
	private static int setLanguage(Context context) {
	...
	private static int setUserName(Context context) throws Exception {
		userName = EnoviaResourceBundle.getProperty(context, LCD_3DX_SAP_INTEGRATION_KEY,
				"LCD_3DXSAPStringResource_en.SAP.UserName", getLanguage());
		return 0;
	}
	...
	private static int setPassword(Context context) throws Exception {
		password = EnoviaResourceBundle.getProperty(context, LCD_3DX_SAP_INTEGRATION_KEY,
				"LCD_3DXSAPStringResource_en.SAP.Password", getLanguage());
		return 0;
	}
	...
	private static int setURL(Context context) throws Exception {
		url = EnoviaResourceBundle.getProperty(context, LCD_3DX_SAP_INTEGRATION_KEY,
				"LCD_3DXSAPStringResource_en.SAP.WebServiceURL.DEV", getLanguage());
		return 0;
	}
	 ...
	 * @throws Exception
	 */
	private static int setChangeActionJson(JsonObject changeActionJson) {
		changeActionJson = changeActionJson;
		return 0;
	}
	 ...
	 * @throws Exception
	 */
	private static int setCloseableHttpClient() {
	 ...
	 * @throws Exception
	 */
	private static int setCSRFToken(String xcsrftoken) {
	 ...
	 * @throws Exception
	 */
	private static int setSessionCookie(String strCookie) {
		cookie = strCookie;
		return 0;
	}
	...
	private String callGETService(Context context) throws Exception {
		...
		try {
			...
			String authorization = EnoviaResourceBundle.getProperty(context, LCD_3DX_SAP_INTEGRATION_KEY,
					"LCD_3DXSAPStringResource_en.SAP.Authorization", getLanguage());
			String xcsrfToken = EnoviaResourceBundle.getProperty(context, LCD_3DX_SAP_INTEGRATION_KEY,
					"LCD_3DXSAPStringResource_en.SAP.XCSRFTOKEN", getLanguage());
	...
	private void LoggerInitialization(Context context) throws Exception {
		try {
			String strCompleteLogger = EnoviaResourceBundle.getProperty(context, LCD_3DX_SAP_INTEGRATION_KEY,
					"LCD_3DXSAPStringResource_en.logger.CompleteLogger", getLanguage());
			String strJSONLogger = EnoviaResourceBundle.getProperty(context, LCD_3DX_SAP_INTEGRATION_KEY,
					"LCD_3DXSAPStringResource_en.logger.JSONLogger", getLanguage());
...
//					"LCD_3DXSAPStringResource_en.logger.DateFormat", getLanguage());
			String strExtension = EnoviaResourceBundle.getProperty(context, LCD_3DX_SAP_INTEGRATION_KEY,
					"LCD_3DXSAPStringResource_en.logger.Extension", getLanguage());
			String strUnderscore = EnoviaResourceBundle.getProperty(context, LCD_3DX_SAP_INTEGRATION_KEY,
					"LCD_3DXSAPStringResource_en.logger.UnderScore", getLanguage());
			...
			String strLogPath = EnoviaResourceBundle.getProperty(context, LCD_3DX_SAP_INTEGRATION_KEY,
					"LCD_3DXSAPStringResource_en.logger.logPath", getLanguage());
