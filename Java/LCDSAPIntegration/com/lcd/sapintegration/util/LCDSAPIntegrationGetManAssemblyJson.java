...
public class LCDSAPIntegrationGetManAssemblyJson {
	...
	private static JsonObjectBuilder ChangeActionJSONPayload(matrix.db.Context context, String caId,
		...
		try {
			...
			String strCACompletionDate = mcaDetails
					.get(lcdSAPInteg3DExpConstants.ATTRIBUTE_CHANGE_ACTION_RELEASED_DATE);
			...
			if (UIUtil
					.isNotNullAndNotEmpty(mcaDetails.get(lcdSAPInteg3DExpConstants.ATTRIBUTE_CHANGE_ACTION_CHANGETYPE)))
	...
	private static List<String> getRealizedItemList() throws Exception {
		List<String> lstRealizedChanges = new ArrayList<String>();
		if (!LIST_REALIZED_CHANGES.isEmpty()) {
